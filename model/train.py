...
$ train.py --config simple_model.yaml

To run with DDP on 4 gpus on 1 node, example:
$ torchrun --nproc-per-node=4 train.py --config simple_model.yaml
"""
...
class TrajectoryDataset(Dataset):
    def __init__(self, paths, augment, uniform):
        self.paths = paths
        self.action_bins = torch.tensor(np.linspace(-1, 1, config['num_action_bins'] - 1))
    ...
    def __getitem__(self, idx):
        ...
        vals = tokens[self.infos[dataset_name]['num_init_steps']:step_count, :dim].contiguous()

        bucketized = torch.bucketize(vals, self.action_bins, right=False)
        bucketized = bucketized + 1

        epsilon = 1e-6
        bucketized[(vals >= -1.0 - epsilon) & (vals <= -1.0 + epsilon)] = 0
        bucketized[(vals >= 1.0 - epsilon) & (vals <= 1.0 + epsilon)] = config['num_action_bins'] - 1
        target_actions[self.infos[dataset_name]['num_init_steps']:step_count, :dim] = bucketized
